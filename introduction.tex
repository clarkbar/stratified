...
\begin{itemize}
	\item when $ X $ is a topological space, and $ A $ is the family of all sheaves (\Cref{sub:notes_on_sobriety});
...
\begin{dfn}
	...
	Then we say that $ X $ is \defn{sober} if the map $ X \to I(X) $ given by $ x \mapsto \xbar $ is a bijection;
...
\begin{lem}\label{asetisitsownshape}
...
The first thing we can notice is that this functor, though not always representable as in \Cref{asetisitsownshape}, is always \emph{prorepresentable};
...
\begin{nul}
	Let us consider the category $ \Top^{\textit{ng}} $ of \defn{numerically generated} topological spaces -- these are topological spaces $ X $ with the property that a subset $ U \subseteq X $ is open if and only if, for any continuous map $ \phi \colon \RR^n \to X $, the set $ \phi^{-1} U $ is open.
